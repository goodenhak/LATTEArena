...
class MonteCarloSearch(TreeofThoughts):
    ...
    def monte_carlo_search_feature_generation(
        ...
        if search_strategy not in {"mcts", "greedy"}:
            raise ValueError(
                f"Unsupported search strategy: {search_strategy}. "
                "Choose from {'mcts', 'greedy'}."
            )
        ...
        for step in range(1, max_steps + 1):
            ...
            for state, new_train, new_val, new_test, metadata in current_states:
                ...
                for thought, (value, new_accuracy, new_train, new_val, new_test, metadata) in evaluated_thoughts.items():
                    ...
                    if (
                        ...
                        if search_strategy == "mcts":
                            is_selected = (
                                state == initial_prompt and ucb1_value >= pruning_threshold
                            ) or (
                                state != initial_prompt
                                and value > state_values[state]
                                and ucb1_value >= pruning_threshold
                            )
                        else:
                            is_selected = (
                                state == initial_prompt and value >= pruning_threshold
                            ) or (
                                state != initial_prompt
                                and value > state_values[state]
                                and value >= pruning_threshold
                            )
                ...
                if search_strategy == "greedy" and candidate_states:
                    best_candidate_value = max(candidate[1] for candidate in candidate_states)
                    tied_candidates = [
                        candidate
                        for candidate in candidate_states
                        if np.isclose(candidate[1], best_candidate_value)
                    ]
                    candidate_states = [random.choice(tied_candidates)]

                for flattened_state, value, new_accuracy, new_train, new_val, new_test, metadata in candidate_states:
                    logger.info(f"\n--- Round: {step}, Depth: {depth} ---")
            ...
            if search_strategy == "greedy":
                if selected_states:
                    ...
                    depth += 1
            elif len(selected_states) >= max_states:
                current_states = selected_states
